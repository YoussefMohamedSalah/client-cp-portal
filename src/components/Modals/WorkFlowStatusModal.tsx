...
function WorkFlowStatusModal<
  T extends
    ...
    | Contract
    | Invoice,
>({ open, handleClose, selectedDocument }: Props<T>) {
            ...
            <div className="row text-center justify-content-center mb-5">
              <div className="col-xl-6 col-lg-8">
                ...
                <p className="text-muted">
                  ...
                  <br />
                  Note: Request Is Approved only if all points is in <span style={{ color: "green" }}>Green</span>{" "}
                  Color.
                  <br />
                    ...
                    selectedDocument?.work_flow?.map((workFlow, index: number) => {
                      return (
                        <div className="timeline-step" key={index}>
                          <div className="timeline-content" data-original-title={`${workFlow.title}`}>
                            <div
                              className={`inner-circle  ${
                                workFlow.state === false && workFlow.isRejected === false
                                  ? "pending"
                                  : `${workFlow.isRejected === true ? "rejected" : "accepted"}`
                              } `}
                            />
                            <p className="h6 mt-3 mb-1">{workFlow.title}</p>
                            <p className="h6 text-muted mb-0 mb-lg-0">{workFlow.name}</p>
                          </div>
                        </div>
                      );
                    })}
