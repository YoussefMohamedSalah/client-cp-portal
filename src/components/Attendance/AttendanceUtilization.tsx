...
const AttendanceUtilization: React.FC<Props> = ({ latitude, longitude }) => {
    ...
    const handleStartWork = async () => {
        try {
            if (session && session?.user?.department_info! && session?.user?.department_info?.name! !== ENUMS.DEPARTMENTS.PROJECTS) {
                const createInput = {
                    latitude: '',
                    longitude: '',
                    lateReason: ''
    ...
    if (attendanceData.msg) return (
        <div className="container-xxl d-flex align-items-center justify-content-center" style={{ minHeight: ' 70vh' }}>
            <div className="text-center row">
                <div className=" col-md-12 mt-5">
                    <p className="fs-3"> <span className="text-danger">Oops! </span>{attendanceData.msg}</p>
                    {/* Chat With Hr */}
                    <p className="fs-5">
                        You Can Inform The HR About That Issue.
                    </p>
                    <div className="d-flex col align-items-center justify-content-center gap-3">
                        ...
                        >
                            <i className="icofont-chat fs-5 me-2" />Talk To HR
                        </Link>
                        <Link
                            to="/"
                            className="btn btn-primary"
                        >
                            <i className="icofont-home fs-5 me-2" />Go Home
                        </Link>
                    </div>
                </div>
            </div>
