    ...
    >
      <div className="d-flex flex-column h-100">
        ...
        >
          ...
          <span className="logo-text">{session?.company?.name ? session?.company?.name : "Co name"}</span>
        </span>
        <ul className="menu-list flex-grow-1 mt-3">
          {menuArray.map((data: any, index: any) => {
            if (!checkRole(data.roles!, data.departments!, session!)) return null;
            if (data.isToggled) {
              return (
                <li key={"nav" + index}>
                  <span
            ...
            return (
              <li key={"subNav" + index} className=" collapsed">
                <div
                  className={`m-link pointer ${
                    data.children.filter((d: any) => "/" + data.routerLink[0] === activeKey).length > 0 ? "active" : ""
                  }`}
                  onClick={(e) => {
                    e.preventDefault();
