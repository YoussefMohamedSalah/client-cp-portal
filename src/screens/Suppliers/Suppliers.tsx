    ...
    let columnT: any[] = [
        {
            ...
            sortable: true,
            cell: (row: any) => <span onClick={() => push(`/${PAGES.SUPPLIER_INFO}/${row.id}`)} className="fw-bold text-secondary pointer">{row.code}</span>
        },
        {
            name: "NAME",
            selector: (row: any) => { },
        ...
        },
        {
            ...
            sortable: false,
            cell: (row: any) =>
                <TableActionBtn id={row.id!} title={row.name} onDelete={handleDelete} onClickEdit={() => push('/' + PAGES.SUPPLIER + '/' + row.id)} />
        }
    ];
