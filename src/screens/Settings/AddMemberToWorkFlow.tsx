...
const AddMemberToWorkFlow: React.FC<Props> = ({ onClose, show, onSelect, selectedEmployees }) => {
  ...
  const { data: employeesData, error: employeeError, isLoading: employeeIsLoading } = useEmployeesQuery({});
                        ...
                        <div className="flex-fill ms-3 text-truncate">
                          ...
                          <span className="text-muted">
                            {employee?.groups?.length!
                              ? `Member In ${employee?.groups?.length!} Group${employee?.groups?.length! > 1 ? "s" : ""}`
                              : ""}{" "}
                          </span>
                          <span className="text-muted">
