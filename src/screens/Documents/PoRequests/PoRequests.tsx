...
const PcRequests: React.FC = () => {
	...
	let columnT: any[] = [
		...
		},
		{
			...
			sortable: true,
			cell: (row: any) =>
				<span className="fw-bold ms-1">{row.project_details.name!}</span>
		},
		{
			name: "Rev",
			width: "70px",
			...
			sortable: true,
			cell: (row: any) =>
				<span className="fw-bold ms-1">{row.rev_num!}</span>
		},
		{
			name: "User",
			width: "200px",
			...
			sortable: true,
			cell: (row: any) =>
				<span className="fw-bold ms-1">{row.user?.name!}</span>
		},
		{
			name: "Description",
			width: "240px",
	...
	return (
		<>
			...
			{selectedDocument && selectedDocument.status !== ENUMS.STATUS.ARCHIVED && <WorkFlowStatusModal<PurchaseOrderRequest> handleClose={handleClose} open={open} selectedDocument={selectedDocument} />}
		</>
	);
};
