...
const PoFormPage = ({ id }: Props) => {
    ...
    const [installments, setInstallments] = useState<PrintInstallments[]>([{ name: "", percentage: 0, value: 0, details: "", date: "" }]);
    // ------
    ...
    const formFields: IField[] = [
        ...
        },
        {
            ...
            onChange: (value: string) =>
                handleModelData(PoRequestKeys.AVAILABILITY, value),
        ...
        },
        {
            ...
            onChange: (value: string) =>
                handleModelData(PoRequestKeys.TRANSPORTATION, value),
        ...
        },
        {
            ...
            onChange: (value: any) =>
                handleModelData(PoRequestKeys.DELIVERY_DATE, value),
            placeholder: "",
            required: true,
        },
        {
            ...
            onChange: (value: string) =>
                handleModelData(PoRequestKeys.DESCRIPTION, value),
...
we would like to place the purchase order for Below Items.`,
    ...
    const handleRemoveOldFile = (file: FileType) => {
        if (file.state === undefined || file.state === false) {
            const selectedFile: FileType | undefined = oldFiles.find((selectedFileToAdd: FileType) => selectedFileToAdd.name === file.name);
            if (selectedFile) {
                ...
                setRemovedFilesNameSet((prevSet) =>
                    prevSet.filter((selectedFile) => selectedFile !== file.name)
                );
            }
        } else {
            const filteredFilesArr: FileType[] = oldFiles.filter(
                (selectedFile: FileType) => selectedFile.name !== file.name
            );
            setOldFiles([...filteredFilesArr, { ...file, state: false }]);
            setRemovedFilesNameSet((prevSet) =>
                prevSet.includes(file.name) ? prevSet : [...prevSet, file.name]
            );
        }
    };
    ...
    const handleAddItem = () => {
        setItems((prevItems) => [
            ...prevItems,
            { description: "", item: "", count: "", price: "", total: "0" }
        ]);
    };
        ...
        <div className="container-xxl">
            ...
            <div className="row g-3 pb-3 pb-xl-0">
                <div>
                    ...
                    {(!isEdit && defaultConditions.length !== 0) ? <DefaultConditionsCard defaultConditions={defaultConditions} onChange={handleDefaultConditionsChange} /> : null}
                    <ConditionsCard conditions={conditions} onAddCondition={handleAddCondition} onEdit={handleConditionChange} onRemove={handleRemoveCondition} />
                    <ItemsCard
                        items={items}
                    ...
                    />
                    <InstallmentsCard total={totalAmount} onChange={handleInstallmentsChange} initialInstallments={installments} initialPaymentType={paymentType} />
                ...
                <div className="col-sm d-flex align-items-center justify-content-center gap-2">
                    ...
                    {isEdit ? (<>
                        {allowEditActionBtn(documentData?.poRequestDetails?.data?.user?.id!, documentData?.poRequestDetails?.data!) && (
