...
const Customers: React.FC = () => {
  ...
  let customers: Customer[] = data.customers.data! || ([] as Customer[]);
  ...
  let columnT: any[] = [
    {
      ...
      sortable: true,
      cell: (row: any) => (
        <span onClick={() => push(`/${PAGES.CUSTOMER_INFO}/${row.id}`)} className="fw-bold text-secondary pointer">
          {row.code}
        </span>
      ),
    },
    ...
    },
    {
      ...
      cell: (row: any) => (
        <TableActionBtn
          ...
          onClickEdit={() => push("/" + PAGES.CUSTOMER + "/" + row.id)}
        />
      ),
    },
  ];
